...
\begin{frame}{Equivalence Principle}
	...
	\end{itemize}
	As an example consider a termn insurance where we define the loss as
	$$L = C \cdot v^{k+1} \mathbbm{1}_{k<n} - \Pi \cdot \ddot{a}_{\min (k,n)}$$
	By the Equivalence Pricinple we have $\mathbb{E}[L] = 0$ which leads to $$ \Pi\cdot \ddot{a}_{x:\overline{n}|} = \Pi \cdot A_{x:\overline{n}|}^{\quad 1}.$$
\end{frame}


\begin{frame}{Mathemical Reserves}
	The Mathematical Reserves V at given time $t$ are defined as \\  
	V = PV(future benefits) - PV(future premiums)
	\vskip 0.5cm
...
\begin{frame}{2.5 Which equivalence principle is fulfilled for the second premium assuming only one premium is paid.}
	\begin{itemize}
		...
		\item From time $t=1$ on until maturity the insruer is at risk of $\prescript{}{2}{\tilde{L}}_x$
		\item By the Equivalence Principle $\mathbb{E}(premium) = \mathbb{E}(benefits)$
	...
	\begin{itemize}
		\item 	Similar to before we compute the benefits:
				\begin{align*}
					...
					&= v \prescript{}{1}{\tilde{L}} \cdot A_{x+1 : \overline{9}|}
				\end{align*}

		\item A short check in the jupyter notebook shows that $\mathbb{E}(premium) = \mathbb{E}(benefits)$ as well as the euqality is exactly the defintion for the reduced benefit $\prescript{}{1}{\tilde{L}}$.
	\end{itemize}
...
\begin{frame}{3.4 MC Averages Analysis}
	...
	\begin{figure}
		\includegraphics[width=1.0\textwidth]{Mean_Reserve_Estimates_with_SEM_Markov_States_34.png}
	\end{figure}
\end{frame}



\begin{frame}{3.4 CDF for 100'000 Simulations}
	\begin{figure}
		\includegraphics[width=0.7\textwidth]{Empirical_CDF_Markov_States_34.png}
	\end{figure}
...
\begin{frame}{3.5 Discussion of Risk Levels of States}
	In order to dicuss the riskiness of the states we look at the coefficient of variation  of the reserve in each state\\
	\begin{figure}
		\includegraphics[width=0.7\textwidth]{Coefficient_of_Variation_Markov_States_35.png}
	\end{figure}
	% \begin{itemize}
	% 	\item 	{ \bfseries Rule of thumb}: Under identical mortality and disability assumptions, insured states associated with higher mathematical reserves are riskier for the insurer.
	% 	\item The state $(D,D)$ is thus the riskiest state since both persons are disabled and with no possibility of reactivation. Hence the insurer pays the highest possible annuity of $18'000$ per year.
	% 	\item We sort the states according to their risk level (from riskiest to safest):
	% 		\begin{align*}
	% 			(3,3) &> (D,D) >  (3,S) > (3,T) > (D,T) > (D,S) \\
	% 			&> (S,3) > (T,3) > (S,D) > (T,D) > (S,S)
	% 		\end{align*}
	% \end{itemize}
\end{frame}
...
\begin{frame}{3.5 Discussion of Risk Levels of States}
	\begin{itemize}
		\item The state $(D,T)$ is the most dangerous state (has highest CoV) since the man is disabled and rather young (30y).
			 Hence he is has three possibilities reactiate, remain disabled for extended periods.
		\item The diversification from having two people in similar but independent states reduces relative variability
		\item The state $(3,3)$ is the safest one since both personsn
	\end{itemize}	
\end{frame}

\begin{frame}{3.6}{Dicussion of Changes in Risk when changing }
...
\begin{frame}{3.8: 8-policy portfolio simulation}
	\begin{itemize}
		...
		\item Now we want to compute the stop loss premium $\Pi(\beta)$ for the deductibles $\beta \in \{1.5\cdot \mathbb{E}[L], 3 \cdot \mathbb{E}[L]\}$
...
\begin{frame}{3.8: 8-policy portfolio simulation}
	Inser pciture of histogram here
\end{frame}
...
\begin{frame}{3.8: 8-policy portfolio simulation}
	...
	\begin{itemize}
		...
		\item Plugging in the values for $\beta$ we get:
			\begin{align*}
				\Pi(1.5\cdot \mathbb{E}[L]) &= \\
				\Pi(3\cdot \mathbb{E}[L]) &=
			\end{align*}
	\end{itemize}
\end{frame}
% \section{Appendix}

% \paragraph{Histogram setup.}
% Let the bins of the histogram be
% \[
% [a_1,a_2),\;[a_2,a_3),\;\dots,\;[a_N,a_{N+1}) .
% \]
% Assume the survival function is constant on each bin:
% \[
% \bar F(x) = 1 - F(x) \approx \bar F_i 
% \quad \text{for } x \in [a_i,a_{i+1}) ,
% \]
% where $\bar F_i$ denotes the value of the survival function on bin $i$.

% \paragraph{Discrete approximation.}
% The integral representation
% \[
% \Pi(\beta) = \int_{\beta}^{\infty} \bar F(x)\,dx
% \]
